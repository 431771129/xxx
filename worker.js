  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/api/profile" && method === "GET") {
        ...
        return new Response(JSON.stringify(profile || { stars: 0, current_grade: 'preschool', speech_rate: 0.9, speech_pitch: 1.0 }), { headers: corsHeaders });
      }
      ...
      if (path === "/api/tasks" && method === "POST") {
        const body = await request.json();
        const { task_name, reward_stars } = body;
      ...
      if (path === "/api/rewards/redeem" && method === "POST") {
        const body = await request.json();
        const cost = body.cost ?? body.cost_stars; // 兼容 cost 和 cost_stars 两个字段名
        
        if (cost === undefined || cost === null) {
          return new Response(JSON.stringify({ error: "参数缺少兑换所需星星数" }), { status: 400, headers: corsHeaders });
        }

        const profile = await env.DB.prepare("SELECT stars FROM user_profile WHERE id = 1").first();
        
        if (!profile) {
          return new Response(JSON.stringify({ error: "找不到用户数据" }), { status: 404, headers: corsHeaders });
        }
